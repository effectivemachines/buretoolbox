...
if [[ -f /testptch/console.txt ]]; then
  OVERWRITEARGS+=("--console-report-file=/testptch/console.txt")
fi

if [[ -f /testptch/brief.txt ]]; then
  OVERWRITEARGS+=("--brief-report-file=/testptch/brief.txt")
fi

if [[ -f /testptch/report.htm ]]; then
  OVERWRITEARGS+=("--html-report-file=/testptch/report.htm")
fi

cd "${PATCH_DIR}/precommit/" || exit 1
"${PATCH_DIR}/precommit/test-patch.sh" "${OVERWRITEARGS[@]}"
