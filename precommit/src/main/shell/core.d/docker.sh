...
function docker_usage
{
  ...
  if [[ "${DOCKER_CLEANUP_CMD}" == false ]]; then
    ...
    yetus_add_option "--dockerplatform=<plat>" "Use a platform string for building and pulling (default: ${DOCKER_PLATFORM})"
    yetus_add_option "--dockerprivd=<bool>" "Run docker in privileged mode (default: '${DOCKER_ENABLE_PRIVILEGED}')"
    yetus_add_option "--dockersocket=<socket>" "Mount given socket as /var/run/docker.sock into the container when Docker-in-Docker mode is enabled (default: '${DOCKER_SOCKET}')"
    yetus_add_option "--dockertag=<tag>" "Use the given Docker tag as the base"
  fi
